...
"""
    ...
    if system_info.cpu:
        ...
        print(f"│ Cores:        {system_info.cpu.physical_cores or '?'} physical, {system_info.cpu.logical_cores or '?'} logical")
...
def main() -> None:
    ...
    # 🚨 ИСПРАВЛЕННАЯ ЛОГИКА: Собираем инфо только если секция есть, но баннер печатаем ВСЕГДА
    system_info = None
    if config.system_info is not None:
        collector = HardwareCollector(config.system_info)
        system_info = collector.get_system_info()

    # Печатаем баннер в любом случае
